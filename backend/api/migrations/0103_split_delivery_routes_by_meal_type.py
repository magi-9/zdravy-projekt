# Raňajky/obed/olovrant dostávajú vlastné, na sebe nezávislé trasy a bloky
# (#dashboard-per-meal-routes) — doteraz mala Prevádzka jednu spoločnú
# `delivery_route`/`delivery_sort_order` pre všetky jedlá.
#
# Doterajšia trasa/blok sa považuje za obedovú (tak sa reálne používala) —
# skopíruje sa do `delivery_route_lunch`/`delivery_sort_order_lunch`, raňajky
# a olovrant začínajú bez trasy (admin si ich nastaví v "Trasy").

import django.db.models.deletion
from django.db import migrations, models


def copy_delivery_route_to_lunch(apps, schema_editor):
...
def copy_lunch_to_delivery_route(apps, schema_editor):
    Prevadzka = apps.get_model("api", "Prevadzka")
...
class Migration(migrations.Migration):
    ...
    operations = [
        ...
        migrations.RunPython(
            copy_delivery_route_to_lunch, copy_lunch_to_delivery_route
        ),

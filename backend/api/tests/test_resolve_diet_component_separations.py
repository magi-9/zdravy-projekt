"""`resolve_diet_component_separations` (#568, flip 10.9.2026) — číta
`DietComponentMerge` pre daný deň do lookup tvaru, ktorý `gramage_dashboard`
priamo použije (ako "zvlášť" výnimky z default "spolu")."""
...
from api.services.meal_plan_service import resolve_diet_component_separations

pytestmark = pytest.mark.django_db


def test_no_rows_for_the_day_returns_empty_dict():
    assert resolve_diet_component_separations("2026-09-10") == {}
...
def test_groups_component_indices_by_meal_and_diet_name():
    ...
    separations = resolve_diet_component_separations("2026-09-10")

    assert separations == {("main_course", "Bez lepku"): {0, 1}}
